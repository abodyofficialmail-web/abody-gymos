-- SHI005: 宮川裕充 / myahrhr@gmail.com（新宿）
-- LINE連携: 新宿店LINE公式で会員番号「SHI005」を送信 → 案内に従い「はい」で members.line_user_id が保存される
...
SELECT
  ...
  now(),
  now()
WHERE NOT EXISTS (SELECT 1 FROM public.members m WHERE m.member_code = 'SHI005');
