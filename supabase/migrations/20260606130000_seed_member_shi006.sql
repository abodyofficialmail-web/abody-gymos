-- SHI006: 澤田有人夢 / yourenmengz@gmail.com（新宿）
-- LINE連携: 新宿店LINE公式で会員番号「SHI006」を送信 → 案内に従い「はい」で members.line_user_id が保存される
...
SELECT
  ...
  '澤田有人夢',
  '澤田有人夢',
  'yourenmengz@gmail.com',
  true,
  (SELECT id FROM public.stores WHERE name = '新宿' LIMIT 1),
  now(),
  now()
WHERE NOT EXISTS (SELECT 1 FROM public.members m WHERE m.member_code = 'SHI006');
