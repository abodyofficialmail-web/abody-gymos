/**
 * member_weight_logs / morning_weight_reminder_dispatches を作成する。
 *
 *   SUPABASE_DB_PASSWORD='...' node scripts/apply-member-weight-logs-migration.mjs
 *   DATABASE_URL='postgresql://...' node scripts/apply-member-weight-logs-migration.mjs
 */
import fs from "fs";
...
const sqlPath = path.join(root, "supabase/migrations/20260829120000_member_weight_logs.sql");
...
async function applyWithPg() {
  ...
  try {
    ...
    const { rows } = await client.query("select to_regclass('public.member_weight_logs') as t");
...
async function tableExists(serviceKey, supabaseUrl) {
  const res = await fetch(`${supabaseUrl}/rest/v1/member_weight_logs?select=id&limit=1`, {
...
async function main() {
  ...
  if (serviceKey && supabaseUrl && (await tableExists(serviceKey, supabaseUrl))) {
    console.log("member_weight_logs は既に存在します。");
    return;
  }

  if (await applyWithPg()) return;

  console.log("=== member_weight_logs マイグレーション（手動） ===\n");
