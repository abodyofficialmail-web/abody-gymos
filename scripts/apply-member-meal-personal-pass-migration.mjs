/**
 * members に食事パーソナル Stripe パス用カラムを追加する。
 *
 *   SUPABASE_DB_PASSWORD='...' node scripts/apply-member-meal-personal-pass-migration.mjs
 *   DATABASE_URL='postgresql://...' node scripts/apply-member-meal-personal-pass-migration.mjs
 */
import fs from "fs";
...
const sqlPath = path.join(root, "supabase/migrations/20260916120000_member_meal_personal_pass.sql");
...
async function columnExists(serviceKey, supabaseUrl) {
  const res = await fetch(`${supabaseUrl}/rest/v1/members?select=meal_personal_pass_status&limit=1`, {
...
async function applyWithPg() {
  ...
  try {
    ...
    const { rows } = await client.query(
      "select column_name from information_schema.columns where table_schema='public' and table_name='members' and column_name='meal_personal_pass_status'"
    );
...
async function main() {
  ...
  if (serviceKey && supabaseUrl && (await columnExists(serviceKey, supabaseUrl))) {
    console.log("meal_personal_pass_status は既に存在します。");
    return;
  }

  if (await applyWithPg()) return;

  console.log("=== meal personal pass マイグレーション（手動） ===\n");
