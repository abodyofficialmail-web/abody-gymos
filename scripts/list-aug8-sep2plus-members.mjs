/**
 * 8月8回以上利用 かつ 9月2枠以上予約の会員を抽出
 * node --env-file=.env.local scripts/list-aug8-sep2plus-members.mjs
 */
...
async function main() {
  ...
  for (const [memberId, augList] of augByMember) {
    ...
    if (sepSlotCount < MIN_SEP_SLOTS) continue;
    ...
    results.push({
      ...
      augSessionCount,
      augSlotCount,
      sepReservationCount: sepList.length,
      sepSlotCount,
      sepBookings,
    });
  }
  ...
  console.log("\n--- 一覧 ---");
  console.log("| # | 会員コード | 氏名 | 所属店 | 8月利用 | 8月枠 | 9月枠 | 9月予約数 |");
  console.log("|---:|---|---|---|---:|---:|---:|---:|");
  results.forEach((r, i) => {
    console.log(
      `| ${i + 1} | ${r.memberCode} | ${r.displayName} | ${r.homeStore ?? "—"} | ${r.augSessionCount} | ${r.augSlotCount} | ${r.sepSlotCount} | ${r.sepReservationCount} |`,
    );
  });
