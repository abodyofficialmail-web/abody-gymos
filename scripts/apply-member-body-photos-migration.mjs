/**
 * member_body_photo_sets テーブルと Storage バケットを Supabase に作成する。
 *
 * 使い方（DBパスワードがある場合）:
 *   SUPABASE_DB_PASSWORD='your-db-password' node scripts/apply-member-body-photos-migration.mjs
 *
...
const sqlPath = path.join(root, "supabase/migrations/20260609120000_member_body_photo_sets.sql");
...
async function applyWithPg() {
  ...
  try {
    ...
    const { rows } = await client.query(
      "select to_regclass('public.member_body_photo_sets') as table_name"
    );
...
async function checkTable(serviceKey, supabaseUrl) {
  const res = await fetch(`${supabaseUrl}/rest/v1/member_body_photo_sets?select=id&limit=1`, {
...
async function main() {
  ...
  if (serviceKey && supabaseUrl && (await checkTable(serviceKey, supabaseUrl))) {
    console.log("member_body_photo_sets テーブルは既に存在します。");
    return;
  }
  ...
  console.log("=== member_body_photo_sets マイグレーション（手動） ===\n");
  console.log("本番DBにテーブルがありません。Supabase Dashboard → SQL → New query に貼り付けて Run:\n");
  console.log(sql);
  console.log(
    "\nまたは DB パスワードを指定して再実行:\n  SUPABASE_DB_PASSWORD='...' node scripts/apply-member-body-photos-migration.mjs"
  );
}
