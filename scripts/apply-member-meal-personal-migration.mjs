 ...
 * 食事パーソナル用テーブルを作成する。
 *
 *   SUPABASE_DB_PASSWORD='...' node scripts/apply-member-meal-personal-migration.mjs
 */
import fs from "fs";
...
const sqlPath = path.join(root, "supabase/migrations/20260830140000_member_meal_personal.sql");
...
async function tableExists(serviceKey, supabaseUrl) {
  const res = await fetch(`${supabaseUrl}/rest/v1/member_meal_logs?select=id&limit=1`, {
...
async function applyWithPg() {
  ...
  try {
    ...
    console.log("OK: meal personal tables");
    return true;
  } finally {
    await client.end();
  }
...
async function main() {
  ...
  if (serviceKey && supabaseUrl && (await tableExists(serviceKey, supabaseUrl))) {
    console.log("member_meal_logs は既に存在します。");
    return;
  }
  if (await applyWithPg()) return;
  console.log("=== 食事パーソナル マイグレーション（手動） ===\n");
