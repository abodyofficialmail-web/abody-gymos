...
function buildRows() {
  ...
  // たけはる 新宿 9/17-（170h目標・桜木町384枠のため店舗を新宿へ）
  const takeShinjuku = [
  ...
  for (const [date, start, end] of takeShinjuku) {
    if (TAKE_OFF.has(date)) continue;
    if (KOHEI_DAYS.has(date) && date === "2026-09-17") continue; // 9/17は上野へ振替
    if (KOHEI_DAYS.has(date) && start === "16:00") continue; // こうへい日は新宿PM不可→上野
    rows.push(row(date, start, end, "たけはる", "新宿"));
  }
  // こうへい日の新宿PM分を上野へ（170h目標）
  const takeUenoKohei = [
    ["2026-09-17", "10:00", "13:00"], ["2026-09-17", "16:00", "22:00"],
    ["2026-09-24", "16:00", "22:00"],
    ["2026-09-29", "16:00", "22:00"],
  ];
  for (const [date, start, end] of takeUenoKohei) {
    if (TAKE_OFF.has(date)) continue;
    rows.push(row(date, start, end, "たけはる", "上野"));
  }
