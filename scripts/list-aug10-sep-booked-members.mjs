/**
 * 8月10回以上利用 かつ 9月に予約ありの会員を抽出
 * node --env-file=.env.local scripts/list-aug10-sep-booked-members.mjs
 */
...
async function main() {
  ...
  const candidateIds = [...augByMember.keys()].filter((id) => {
    ...
    return (sepByMember.get(id)?.length ?? 0) >= 1;
  });
    ...
    JSON.stringify(
      {
        ...
        criteria: {
          ...
          september: "2026-09 予約1件以上（キャンセル除外）",
        },
