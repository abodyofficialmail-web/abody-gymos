/**
 * pre_session_survey テーブルを Supabase に作成する。
 *
 *   SUPABASE_DB_PASSWORD='...' node scripts/apply-pre-session-survey-migration.mjs
 */
import fs from "fs";
...
const sqlPath = path.join(root, "supabase/migrations/20260623140000_pre_session_survey.sql");
...
async function applyWithPg() {
  ...
  try {
    ...
    const { rows } = await client.query(
      "select to_regclass('public.pre_session_survey_responses') as responses"
    );
...
async function checkTable(serviceKey, supabaseUrl) {
  const res = await fetch(`${supabaseUrl}/rest/v1/pre_session_survey_responses?select=id&limit=1`, {
...
async function main() {
  ...
  if (serviceKey && supabaseUrl && (await checkTable(serviceKey, supabaseUrl))) {
    console.log("pre_session_survey_responses は既に存在します。");
    return;
  }
  ...
  console.log("=== pre_session_survey マイグレーション（手動） ===\n");
