...
import { sendMonthlyProgressLine } from "@/lib/monthlyProgressLine";
...
const bodySchema = z.object({
  ...
  pdf_file_name: z.string().min(1).optional(),
...
export async function POST(req: Request) {
  try {
    ...
    for (const code of parsed.data.member_codes) {
      results.push(
        await sendMonthlyProgressLine(supabase, {
          memberCode: code,
          text: parsed.data.text,
          imageUrls: parsed.data.image_urls,
          pdfUrl: parsed.data.pdf_url,
          pdfFileName: parsed.data.pdf_file_name,
