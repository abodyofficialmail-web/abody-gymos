...
function mustAuth(req: Request): boolean {
  ...
  const testKey = req.headers.get("x-goal-hearing-test-key") ?? req.headers.get("x-reservation-reminder-test-key") ?? "";
...
export async function POST(req: Request) {
  try {
    ...
    for (const memberCode of memberCodes) {
      const result = await sendGoalHearingInviteForMember(supabase, { memberCode, dryRun });
      results.push(result);
    }
