...
import { sendTyphoonSep21NoticeLine } from "@/lib/typhoonSep21NoticeLine";
...
export async function POST(req: Request) {
  try {
    ...
    const memberCodes = parsed.data.member_codes.map((c) => c.trim().toUpperCase());
    const supabase = createSupabaseServiceClient();
    const results = [];
    for (const code of memberCodes) {
      results.push(
        await sendTyphoonSep21NoticeLine(supabase, {
          memberCode: code,
