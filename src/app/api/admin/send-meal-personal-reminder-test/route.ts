import { z } from "zod";
import { createSupabaseServiceClient } from "@/lib/supabase/admin";
import { defaultMealSlot, deleteMemberMealLogsForSlot, tokyoTodayYmd } from "@/lib/memberMealLogs";
import { sendMealPersonalReminderForCode, type MealReminderSlot } from "@/lib/mealPersonalReminderLine";

function json(body: unknown, status = 200) {
  return new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json" } });
}

function mustAuth(req: Request): boolean {
  const gateSecret = process.env.TRAINER_GATE_SECRET?.trim();
  const cronSecret = process.env.CRON_SECRET?.trim();
  const testKey =
    req.headers.get("x-meal-personal-test-key") ??
    req.headers.get("x-weight-log-test-key") ??
    req.headers.get("x-reservation-reminder-test-key") ??
    "";
  const auth = req.headers.get("authorization") ?? "";
  const bearer = auth.toLowerCase().startsWith("bearer ") ? auth.slice(7).trim() : "";
  if (gateSecret && (testKey === gateSecret || bearer === gateSecret)) return true;
  if (cronSecret && (testKey === cronSecret || bearer === cronSecret)) return true;
  return false;
}

const bodySchema = z.object({
  member_codes: z.array(z.string()).min(1).optional(),
  slot: z.enum(["breakfast", "lunch", "dinner", "snack"]).optional(),
  dry_run: z.boolean().optional(),
});

export async function POST(req: Request) {
  try {
    if (!mustAuth(req)) return json({ error: "unauthorized" }, 401);

    const raw = await req.json().catch(() => ({}));
    const parsed = bodySchema.safeParse(raw);
    if (!parsed.success) return json({ error: "invalid_body", detail: parsed.error.flatten() }, 400);

    const memberCodes = (parsed.data.member_codes ?? ["EBI020"]).map((c) => c.trim().toUpperCase());
    const dryRun = Boolean(parsed.data.dry_run);
    const slot: MealReminderSlot = parsed.data.slot ?? defaultMealSlot();
    const supabase = createSupabaseServiceClient();
    const results: Array<Record<string, unknown>> = [];

    for (const memberCode of memberCodes) {
      results.push(
        await sendMealPersonalReminderForCode(supabase, {
          memberCode,
          slot,
          dryRun,
          force: true,
          recordDispatch: false,
        })
      );
    }

    return json({ ok: true, slot, results }, 200);
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    return json({ error: message }, 500);
  }
}

export async function GET(req: Request) {
  try {
    const url = new URL(req.url);
    const memberCode = (url.searchParams.get("member_code") ?? "EBI020").trim().toUpperCase();
    const slotRaw = url.searchParams.get("slot") ?? "breakfast";
    if (slotRaw !== "breakfast" && slotRaw !== "lunch" && slotRaw !== "dinner" && slotRaw !== "snack") {
      return json({ error: "invalid_slot" }, 400);
    }
    const supabase = createSupabaseServiceClient();
    let cleared = 0;
    if (url.searchParams.get("clear") === "1") {
      const { data: member } = await supabase
        .from("members")
        .select("id")
        .eq("member_code", memberCode)
        .maybeSingle();
      if (member?.id) {
        const deleted = await deleteMemberMealLogsForSlot(supabase, member.id, tokyoTodayYmd(), slotRaw);
        if (!deleted.ok) return json({ error: "clear_failed", detail: deleted.error }, 500);
        cleared = deleted.deleted;
      }
    }
    const result = await sendMealPersonalReminderForCode(supabase, {
      memberCode,
      slot: slotRaw,
      force: true,
      recordDispatch: false,
    });
    return json({ ok: result.sent, slot: slotRaw, cleared, results: [result] }, result.sent ? 200 : 502);
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    return json({ error: message }, 500);
  }
}
