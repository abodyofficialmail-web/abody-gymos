...
export function TrainerDetailClient({ trainerId }: { trainerId: string }) {
  ...
  const loadTrainer = useCallback(async () => {
    const { trainer: t } = await apiGet<{ trainer: TrainerRow }>(`/api/trainers/${encodeURIComponent(trainerId)}`);
    setTrainer(t);
  ...
  const loadSettingsLists = useCallback(async () => {
    const [storesJ, tcJ, exJ] = await Promise.all([
      ...
      apiGet<{ transport_costs: TransportCostRow[] }>(`/api/trainers/${encodeURIComponent(trainerId)}/transport-costs`),
      apiGet<{ expenses: ExpenseRow[] }>(`/api/trainers/${encodeURIComponent(trainerId)}/expenses`),
    ]);
  ...
  const loadBreaks = useCallback(async () => {
    try {
      const j = await apiGet<{ breaks: ShiftBreakRow[] }>(
        `/api/shifts/breaks?trainer_id=${encodeURIComponent(trainerId)}&month=${encodeURIComponent(month)}`
      );
      setBreaks(j.breaks ?? []);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    async function run() {
      setErr(null);
      try {
        const [shiftsData, resData] = await Promise.all([
          apiGet<{ shifts: ShiftRow[] }>(
            `/api/shifts?trainer_id=${encodeURIComponent(trainerId)}&month=${encodeURIComponent(month)}`
          ),
          apiGet<{ reservations: ReservationRow[] }>(
            `/api/booking-v2/reservations?trainer_id=${encodeURIComponent(trainerId)}&month=${encodeURIComponent(month)}`
          ),
          loadTrainer(),
          loadSettingsLists(),
          loadBreaks(),
        ...
        console.error("fetch_failed detail:", error);
        if (cancelled) return;
        setErr(String(error instanceof Error ? error.message : "取得に失敗しました"));
        setShifts((prev) => prev ?? []);
  ...
  }, [trainerId, month, loadTrainer, loadSettingsLists, loadBreaks]);
  ...
  async function saveSettings() {
    ...
    setSaveErr(null);
    try {
      const { trainer: t } = await apiPatch<{ trainer: TrainerRow }>(`/api/trainers/${encodeURIComponent(trainerId)}`, {
  ...
  async function addTransportCost() {
    ...
    try {
      await apiPost<{ transport_cost: TransportCostRow }>(
        `/api/trainers/${encodeURIComponent(trainerId)}/transport-costs`,
  ...
  async function addExpense() {
    ...
    setSaveBusy(true);
    try {
      await apiPost<{ expense: ExpenseRow }>(`/api/trainers/${encodeURIComponent(trainerId)}/expenses`, {
  ...
  const name = trainer?.display_name?.trim() || "（名前未取得）";
  ...
  return (
    <div className="space-y-6">
      ...
      <Link href="/admin/dashboard/trainers" className="text-sm text-slate-600 underline">
        ← 一覧へ
      </Link>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
        ...
        <div className="space-y-2">
          ...
          {(transportCosts ?? []).map((tc) => {
            const st = tc.store_id ? storeById.get(tc.store_id) : null;
            return (
              <div key={tc.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 px-3 py-2">
                <div className="text-sm font-semibold text-slate-900">{st?.name ?? tc.store_id ?? "（店舗未設定）"}</div>
                <input
                  type="number"
                  min={0}
                  step={1}
            ...
            <div key={ex.id} className="grid gap-2 rounded-xl border border-slate-200 px-3 py-2 sm:grid-cols-4 sm:items-center">
                ...
                onBlur={async (e) => {
                  ...
                  try {
                    await apiPatch<{ expense: ExpenseRow }>(
                      `/api/trainers/${encodeURIComponent(trainerId)}/expenses`,
                      { id: ex.id, title: v }
                    );
                    await loadSettingsLists();
                  } catch (err) {
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setSaveErr(null);
                  try {
                    await apiDelete<{ ok: boolean }>(`/api/trainers/${encodeURIComponent(trainerId)}/expenses`, { id: ex.id });
                    await loadSettingsLists();
                  } catch (err) {
                    setSaveErr(String(err instanceof Error ? err.message : "削除に失敗しました"));
                  } finally {
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
        <div className="text-sm font-bold text-slate-900">シフト一覧</div>
        {shifts === null ? <div className="text-sm text-slate-600">読み込み中…</div> : null}
        {(shifts ?? []).length === 0 && shifts !== null ? <div className="text-sm text-slate-600">シフトなし</div> : null}
          ...
          {(shifts ?? []).map((s) => (
            ...
            >
              <div className="font-semibold">
                {s.shift_date} {s.start_local.slice(0, 5)}〜{s.end_local.slice(0, 5)}
              </div>
              <div className="text-xs text-slate-500">
                {storeById.get(s.store_id)?.name ?? s.store_id} / 休憩: {calcTotalBreakMinutes(s.breaks)}分
              </div>
              {s.is_break ? <div className="text-xs font-semibold text-amber-800">休憩</div> : null}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
        <div className="text-sm font-bold text-slate-900">予約一覧</div>
        {reservations === null ? <div className="text-sm text-slate-600">読み込み中…</div> : null}
        {(reservations ?? []).length === 0 && reservations !== null ? <div className="text-sm text-slate-600">予約なし</div> : null}
        <div className="grid gap-2">
          {(reservations ?? []).map((r) => (
